from octopipes.benchmark import Benchmark
from octopipes.dataset import Dataloader, Dataset, InputWithDeps
from octopipes.workflow import Workflow
import pytest

from tests.test_dataset import MockDataset

@pytest.mark.parametrize("mode", ["single","threaded"])
def test_benchmark(mode):
    wf1 = Workflow('test_wf_1')\
            .add(lambda x: x + 1)
    wf2 = Workflow('test_wf_2')\
            .add(lambda x: x * 2)
    dataset: Dataset = MockDataset([1, 2, 3, 4, 5])
    dataloader = Dataloader(dataset=dataset, batch_size=2, drop_last_batch=True)
    benchmark = Benchmark(dataloader=dataloader, workflows=[wf1, wf2],mode=mode)
    benchmark.run_tests()

    sorted_result= sorted(benchmark.results, key=lambda r: r.results[0].output)
    assert len(benchmark.results) == 4
    assert sorted_result[0].results[0].output == 2
    assert sorted_result[0].results[1].output == 2
    assert sorted_result[1].results[0].output == 3
    assert sorted_result[1].results[1].output == 4

@pytest.mark.parametrize("mode", ["single","threaded"])
def test_benchmark_with_dependencies(mode):
    wf1 = Workflow('test_wf_1')\
            .add(lambda x, y: x + y, requires='d0')

    wf2 = Workflow('test_wf_2')\
            .add(lambda x, y: x - y, requires='d0')

    # This dataset defines inputs with dependencies without ground truth
    dataset: Dataset = MockDataset([InputWithDeps(2, dependencies=[2]), InputWithDeps(1, dependencies=[2])])
    dataloader = Dataloader(dataset=dataset, batch_size=2, drop_last_batch=True)
    benchmark = Benchmark(dataloader=dataloader, workflows=[wf1, wf2],mode=mode)
    benchmark.run_tests()
    assert len(benchmark.results) == 2
    assert benchmark.results[0].results[0].output == 4
    assert benchmark.results[0].results[1].output == 0
    assert benchmark.results[1].results[0].output == 3
    assert benchmark.results[1].results[1].output == -1

    # this dataset defines inputs with dependencies with a ground truth value
    dataset: Dataset = MockDataset([
        (InputWithDeps(2, dependencies=[2]), 24),
        (InputWithDeps(1, dependencies=[2]), 24)
    ])
    dataloader = Dataloader(dataset=dataset, batch_size=2, drop_last_batch=True)
    benchmark = Benchmark(dataloader=dataloader, workflows=[wf1, wf2], mode=mode)
    benchmark.run_tests()
    assert len(benchmark.results) == 2
    assert benchmark.results[0].results[0].output == 4
    assert benchmark.results[0].results[1].output == 0
    assert benchmark.results[1].results[0].output == 3
    assert benchmark.results[1].results[1].output == -1


@pytest.mark.parametrize("mode", ["single", "threaded"])
def test_benchmark_large_dataset(mode):
    wf1 = Workflow('wf_add').add(lambda x: x + 1)
    wf2 = Workflow('wf_mul').add(lambda x: x * 2)

    large_dataset = MockDataset(list(range(1000)))  # 1000 samples
    dataloader = Dataloader(dataset=large_dataset, batch_size=100, drop_last_batch=False)

    benchmark = Benchmark(dataloader=dataloader, workflows=[wf1, wf2], mode=mode)
    benchmark.run_tests()

    assert len(benchmark.results) == 1000, "Benchmark should process all 1000 samples"


@pytest.mark.parametrize("mode", ["single", "threaded"])
def test_benchmark_empty_dataset(mode):
    dataset: Dataset = MockDataset([])  # Empty dataset
    dataloader = Dataloader(dataset=dataset, batch_size=2, drop_last_batch=True)
    benchmark = Benchmark(dataloader=dataloader, workflows=[], mode=mode)
    benchmark.run_tests()
    
    assert len(benchmark.results) == 0, "Benchmark should not store results for an empty dataset"



@pytest.mark.parametrize("mode", ["single", "threaded"])
def test_benchmark_with_failing_workflow(mode):
    def failing_function(x):
        raise ValueError("Intentional error!")

    wf1 = Workflow('failing_wf').add(failing_function)
    dataset: Dataset = MockDataset([1, 2, 3])
    dataloader = Dataloader(dataset=dataset, batch_size=2, drop_last_batch=True)
    
    benchmark = Benchmark(dataloader=dataloader, workflows=[wf1], mode=mode)
    
    # Run benchmark, expecting it to handle errors internally
    benchmark.run_tests()
    
    assert len(benchmark.results) == 0, "Benchmark should not store results if all workflows fail"
